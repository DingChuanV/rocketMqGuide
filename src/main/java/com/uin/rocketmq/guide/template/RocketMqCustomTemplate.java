package com.uin.rocketmq.guide.template;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.json.JSONUtil;
import com.alibaba.fastjson.JSONObject;
import com.uin.rocketmq.guide.BaseMqMessage;
import com.uin.rocketmq.guide.Event;
import com.uin.rocketmq.guide.RocketMqDelayLevelEnum;
import com.uin.rocketmq.guide.RocketMqSysConstant;
...
public class RocketMqCustomTemplate {
  ...
  public <T extends BaseMqMessage> SendResult syncSend(String destination, T message) {
    ...
    // 此处为了方便查看给日志转了json，根据选择选择日志记录方式，例如ELK采集
    log.info("[{}]同步消息[{}]发送结果[{}]", destination, JSONUtil.toJsonStr(message),
   ...
   * @return 发送消息结果
   */
  public <T extends BaseMqMessage> SendResult syncSendOrderLy(String destination, T message,
      String hashKey) {
    Message<T> sendMessage = MessageBuilder.withPayload(message)
        .setHeader(RocketMQHeaders.KEYS, message.getKey())
        .build();
    SendResult sendResult = template.syncSendOrderly(destination, sendMessage, hashKey);
    log.info("[{}]同步顺序消息[{}]hashKey[{}]发送结果[{}]", destination,
        JSONUtil.toJsonStr(message), hashKey, JSONUtil.toJsonStr(sendResult));
    return sendResult;
  }
   ...
   * @return 发送消息结果
   */
  public <T extends BaseMqMessage> SendResult syncSendOrderLy(String topic, String tag, T message,
      String hashKey) {
   ...
   * @return <T> 所有 extends BaseMqMessage 的对象
   */
  public <T extends BaseMqMessage> SendResult delaySend(String destination, T message,
      RocketMqDelayLevelEnum delayLevelEnum) {
    Message<T> sendMessage = MessageBuilder.withPayload(message)
        .setHeader(RocketMQHeaders.KEYS, message.getKey()).build();
    SendResult sendResult = template.syncSend(destination, sendMessage, 3000,
        delayLevelEnum.getLevel());
    log.info("[{}]延迟等级[{}]消息[{}]发送结果[{}]", destination, delayLevelEnum.getLevel(),
        JSONUtil.toJsonStr(message), JSONUtil.toJsonStr(sendResult));
    return sendResult;
  }

  public <T extends BaseMqMessage> SendResult syncSendCommonOrderLy(String topic, String tag,
      T message, HashMap<String, String> header, String hashKey) {
